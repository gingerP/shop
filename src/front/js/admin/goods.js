...
var orderWraper = (function getOrderData(grid) {
    function createOrderPack(id, index) {
        return {
            id: id,
            index: index
        }
    ...
    function getOrder(grid) {
        ...
        grid.forEachRow(function (rowId) {
            var entity = grid.getUserData(rowId, 'entity');
            data.push(createOrderPack(entity.id, index));
            index++;
        });
        return data;
    }
