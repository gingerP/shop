...
ImageGallery.prototype.initEvents = function() {
    ...
    $(this.$previewImageSelector).click(
        function () {
            ...
            if (clickedImg[0].id != "item") {
                var next = thiz.getImageIndexByClick(clickedImg[0], $('img', thiz.$previewImageSelector));
                if (thiz.current != next) {
                    thiz.current = next;
                    thiz.changeMainImage(thiz.current);
                }
            }
        }
    )
    ...
    this.$leftArrow.children('div')
        .click(
        function() {
            thiz.selectPrevImage();
        }
    );
    this.$rightArrow.children('div')
        .click(
        function() {
            thiz.selectNextImage();
        }
    );
