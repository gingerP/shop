...
ImageZoom.prototype.init = function (viewPortSelector, viewImageSelector, imageNamePreparator, stateListener) {
    var self = this;
    self.$viewPort = $(viewPortSelector);
    self.$viewImage = $(viewImageSelector);
    self.imageNamePreparator = typeof(imageNamePreparator) === 'function' ? imageNamePreparator : function (data) {
        return data;
    };
    self.stateListener = stateListener;
    self.obj = {};
    self.obj.$zoomSwitcher = undefined;
    self.obj.$zoomWindowContainer = undefined;
    self.obj.$zoomCursor = undefined;
    self.zoomInState = false;
    self.zoomActive = false;
    self.initZoomSwitcher();
    self.__init();
    self.initEvents();
    self.initPosition();
...
ImageZoom.prototype.createCenteredContainer = function createCenteredContainer(content) {
    var template = '\
...
ImageZoom.prototype.__init = function __init() {
    var mainTemplate = '\
    ...
    mainTemplate = mainTemplate.replace('{preview}', readyPreviewTemplate)
        .replace('{zoom}', readyZoomTemplate).replace('{close_button}', readyCloseButtonTemplate);


    this.objId = AuUtils.getRandomString();
...
ImageZoom.prototype.initZoomSwitcher = function () {
    ...
    switcherIcon = document.createElement('div');
    ...
    switcherIcon.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" fill="#414141" height="36" viewBox="0 0 24 24" width="36">\
        <path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 \
        16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 \
...
ImageZoom.prototype.withZoomWindowConfig = function withZoomWindowConfig(config) {
    config = config || {};
    $.extend(true, this.cfg, config);
    return this;
};

ImageZoom.prototype.initEvents = function initEvents() {
    var thiz = this;
    this.obj.$zoomSwitcher.mouseup(
        function (e) {
            ...
            thiz.updateZoomCursorPosition(AuUtils.extractRelativePosition(e, thiz.$viewPort));
            var handlers = {
                imageLoaderCallback: function () {
                    thiz.updateZoomedImagePosition(AuUtils.extractRelativePosition(e, thiz.$viewPort));
                }
            };
    ...
    $('>div', this.obj.$zoomPreviewContainer)
        ...
        function (e) {
            if (thiz.zoomInState) {
                var pos = AuUtils.extractRelativePosition(e, this);
...
ImageZoom.prototype.initPosition = function initPosition() {

};

ImageZoom.prototype.updateZoomCursorPosition = function updateZoomCursorPosition(_pos) {
...
ImageZoom.prototype.updateZoomedImagePosition = function updateZoomedImagePosition(cursorPosition) {
...
ImageZoom.prototype.makeZoomState = function makeZoomState(handlers) {
    this[this.zoomInState ? '_makeZoomInState' : '_makeZoomOutState'](handlers);
    return this;
};

ImageZoom.prototype._makeZoomInState = function _makeZoomInState(handlers) {
    var thiz = this;
...
ImageZoom.prototype._makeZoomOutState = function _makeZoomOutState() {
...
ImageZoom.prototype.getRecPosition = function getRecPosition(pos, viewPortSize, recSize) {
    var newPos = {
        x: 0,
        y: 0
    };
...
ImageZoom.prototype.updatePreviewImage = function updatePreviewImage(src) {
    if (!this.obj.$zoomPreviewImage) {
        return;
    }
    this.obj.$zoomPreviewImage[0].src = src;
};

ImageZoom.prototype.updateZoomedImage = function updateZoomedImage(src) {
    if (!this.obj.$zoomWindowContainer) {
        return;
    }
    this.obj.$zoomWindowContainer.find('img').attr('src', src);
};
